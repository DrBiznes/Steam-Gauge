...
export function CodeBlock({ 
  ...
  return (
    <div className="my-4">
      ...
      >
        <span className="material-symbols-outlined transition-all group-hover:text-[#168f48]">
          {isExpanded ? 'visibility_off' : 'visibility'}
        </span>
        <span className="italic">{title}</span>
        {language && (
          <span className="text-xs px-2 py-1 rounded-full bg-[#168f48]/20 text-[#168f48]">
            {language}
          </span>
        )}
      </button>
            ...
            <div className="border border-[#168f48]/20 overflow-hidden">
              ...
              >
                {({ className, style, tokens, getLineProps, getTokenProps }) => (
                  <pre className={`overflow-x-auto m-0 ${className} font-jetbrains`} style={style}>
